...
if __name__ == "__main__":
    ...
    for rServer in rServers:
        ...
        try:
            rResponse = json.loads(requests.post(rAPI, data=rData, timeout=5).content)[0].split()
        except:
            rFailed = True
            rResponse = []
        if len(rResponse) > 1 and rResponse[0] == "xtreamcodes" and rResponse[1].upper() == "P":
            ...
            try:
                ...
                pass
            if len(rResponse) > 1 and rResponse[0] == "xtreamcodes" and rResponse[1].upper() == "P":
                print("Couldn't disable user password... Please disable it manually on the LB.")
            if int(rServer[0]) != int(rConfig["server_id"]):
                print("You may need to modify /etc/sudoers manually on this load balancer.")
        # Main server only!
        if int(rServer[0]) == int(rConfig["server_id"]):
        ...
        for rLine in rFile.split("\n"):
            if rLine.startswith("echo"):
                rWriteServices = True
            else:
                rNewServices.append(rLine)
        if rWriteServices:
            print(" ")
        ...
        # Report back
        if rFailed:
            print("Server failed to execute some commands, cannot confirm it's not infected!")
        elif not rInfected:
            print("Server not infected!")
        print(" ")
        print(" ")
    ...
    for rServer in rServers:
        if int(rServer[0]) == int(rConfig["server_id"]):
            try:
                import pymysql
                rRDB = pymysql.connect(host=rServer[1], user=rConfig["db_user"], password=rConfig["db_pass"], database=rConfig["db_name"], port=int(rConfig["db_port"]))
            except:
                rRDB = None
            if rRDB:
                ...
                rQ = None
                while rQ not in ["Y", "N"]:
                    rQ = input("Would you like to secure your MySQL? (Enter Y/N) : ").upper()
                if rQ == "Y":
                    ...
                    rQ = None
                    while rQ not in ["Y", "N"]:
                        rQ = input("Would you like to generate a new MySQL password? (Enter Y/N) : ").upper()
                    rNewPass = False
                    if rQ == "Y":
                        rNewPass = True
                        rMySQLPass = generate()
                        ...
                        if rNewPass:
                            ...
                            try:
                                ...
                                if int(rServer[0]) == int(rConfig["server_id"]):
                                    ...
                                    rHost = rServer[1]
                                rOldConfig = {'server_id': str(rServer[0]), 'db_name': rConfig["db_name"], 'pconnect': '0', 'db_port': str(rConfig["db_port"]), 'db_pass': None, 'host': rHost, 'db_user': rConfig["db_user"]}
                ...
                print("MySQL is secure from the outside world! :)")
            break
    print(" ")
    print(" ")
